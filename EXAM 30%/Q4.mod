...
set COMPARTMENTS;	 # 2 compartments
...
#PARAMETERS_NEW_Q4
param compartment_capacity {1..VESSELS, COMPARTMENTS};			# capacity of compartment c in vessel v
...
#DECISION VARIABLES_NEW_Q4
var shipment {1..PRODUCTS, 1..WEEKS, TERMINALS, COMPARTMENTS} integer >= 0;  		# shipping quantities for product p in week w to terminal t 
var W {1..PRODUCTS, TERMINALS, COMPARTMENTS,  1..WEEKS} binary;								# if product p is loaded to compartment c in vessel v
...
s.t. InventInitFact {p in 1..PRODUCTS}					: 	inventory_F[p,1] = initial_inventory_F[p] + production[p,1] - sum{t in TERMINALS, c in COMPARTMENTS} shipment[p,1,t,c];	
s.t. InventEndFact {p in 1..PRODUCTS, w in 2..WEEKS}	:	inventory_F[p,w] = inventory_F[p,w-1] + production[p,w] - sum{t in TERMINALS, c in COMPARTMENTS} shipment[p,w,t,c];	
s.t. StoreLimitFact {p in 1..PRODUCTS, w in 1..WEEKS}	:	inventory_F[p,w] <= inventory_capacity_F[p];

# Inventory Constraints at Terminal
s.t. InventInitTerm {p in 1..PRODUCTS, t in TERMINALS}					:	inventory_T[p,1,t] = initial_inventory_T[p,t] + sum {c in COMPARTMENTS} shipment[p,1,t,c] - sales_forecast[p,1,t];	
s.t. InventEndTerm {p in 1..PRODUCTS, w in 2..WEEKS, t in TERMINALS}	:	inventory_T[p,w,t] = inventory_T[p,w-1,t] + sum {c in COMPARTMENTS} shipment[p,w,t,c] - sales_forecast[p,w,t];
s.t. StoreLimitTerm {p in 1..PRODUCTS, w in 1..WEEKS, t in TERMINALS}	:	inventory_T[p,w,t] <= inventory_capacity_T[p,t];

# Vessels and Compartment Constraints
s.t. CompartmentCap {t in TERMINALS, w in 1..WEEKS, p in 1..PRODUCTS, c in COMPARTMENTS}		
		: shipment[p,w,t,c] <= sum {v in 1..VESSELS} W[p,t,c,w] * compartment_capacity[v, c] *vessel[v,w,t];
s.t. OneProductPerOneCom {v in 1..VESSELS, c in COMPARTMENTS, w in 1..WEEKS, t in TERMINALS}						
		: sum {p in 1..PRODUCTS} W[p,t,c,w] <= 1; # Each compartment can contain only one product at a time

# Transportation Constraints
#s.t. VesselCap {t in TERMINALS, w in 1..WEEKS}			: sum {p in 1..PRODUCTS, c in COMPARTMENTS} shipment[p,w,t,c] <= sum {v in 1..VESSELS} vessel[v,w,t] * vessel_capacity[v];
#s.t. ShipmentCap {w in 1..WEEKS, v in 1..VESSELS}		: sum {t in TERMINALS, p in 1..PRODUCTS,c in COMPARTMENTS} shipment[p,w,t,c] <= sum {t in TERMINALS, p in 1..PRODUCTS}  inventory_F[p,w];
